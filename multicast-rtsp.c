...
static void media_constructed(GstRTSPMediaFactory *factory,
                              GstRTSPMedia *media) {
  guint i, n_streams;
...
int main(int argc, char *argv[]) {
  ...
  GstRTSPMediaFactory *factory1enc = gst_rtsp_media_factory_new();
  GstRTSPMediaFactory *factory5enc = gst_rtsp_media_factory_new();
  GstRTSPMediaFactory *factory10enc = gst_rtsp_media_factory_new();
  GstRTSPMediaFactory *factory30enc = gst_rtsp_media_factory_new();
  GstRTSPMediaFactory *factory1raw = gst_rtsp_media_factory_new();
  GstRTSPMediaFactory *factory5raw = gst_rtsp_media_factory_new();
  ...
  gst_rtsp_media_factory_set_launch(
      factory5raw, "shmsrc socket-path=/tmp/raw-5 do-timestamp=true "
                   "! video/x-raw,format=I420,width=640,height=480 ! "
                   "rtpvrawpay name=pay0 pt=96");
  ...
  g_signal_connect(factory1enc, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory5enc, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory10enc, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory30enc, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory1raw, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory5raw, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory10raw, "media-constructed",
                   (GCallback)media_constructed, NULL);
  g_signal_connect(factory30raw, "media-constructed",
                   (GCallback)media_constructed, NULL);
